...
La collecte utilise exclusivement Patchright et le DOM/HTML rendu, ainsi que le JSON SSR `__NEXT_DATA__` public présent dans la page. Aucun appel direct à l’API SofaScore, endpoint JSON interne ou accès à Stake n’est réalisé par le worker. Les liens sont dédupliqués par identifiant de rencontre ; les pages de journée sont mises en cache cinq minutes, les matchs terminés ne sont pas revisités, et les matchs live connus restent prioritaires avec un intervalle minimal d’une minute. Chaque navigation est séparée par un délai aléatoire borné ; un `403` ou `429` déclenche un arrêt temporaire du collecteur au lieu d’une nouvelle tentative en boucle. Le script `sync-sofascore-matches` est prévu toutes les minutes afin de suivre un direct, tandis que les échéances sont modifiables dans Admin.

La provenance est conservée dans `match_source_links` et chaque état immuable dans `match_snapshots`. Le matching réutilise d’abord un lien SofaScore connu, puis combine équipes normalisées (accents, ponctuation, alias/code/slug), sens de l’affiche, proximité de l’horaire et ligue. Un score insuffisant ou une collision est laissé non résolu et journalisé ; une participation observée ne crée pas d’affiliation historique supposée.

Le contenu SofaScore ne publie pas toujours les détails d’une carte dans le HTML rendu. Le worker tente l’onglet rendu `Matchs`/`Matches` après l’ouverture de la fiche ; dans le cas contraire, `maps` reste vide et l’interface indique l’absence de statistiques. Pour les matchs terminés, Oracle’s Elixir est la source historique complémentaire : les cartes validées (deux camps, cinq joueurs et champs obligatoires) sont conservées dans un snapshot séparé. La lecture API prend le statut et le score du dernier relevé SofaScore, mais reprend le dernier snapshot non vide de cartes afin qu’un relevé live partiel ne remplace jamais un historique déjà présent. Quand elles sont réellement disponibles et validées, le contrat prévoit cartes, bans, joueurs, champions, niveaux, K/D/A, CS, or, tours, dragons, Héraut, larves et inhibiteurs. Ces champs ne sont jamais complétés par des zéros ou des statistiques fictives. Les sources ne constituent ni un calendrier officiel exhaustif, ni une prédiction, ni une preuve de résultat lorsque la page ne l’expose pas.
