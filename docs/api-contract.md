...
L’API actuelle lit `matches` exactement sur la fenêtre J−7/J+7 de Paris. Le statut et le score viennent du dernier snapshot source ; une heure dépassée ne suffit jamais à inférer un direct ou un résultat. MSW fournit le scénario de direct documenté ; le frontend relit la route toutes les 30 secondes quand Matchs est monté. `updatedAt` identifie la date du relevé, distincte de la consultation HTTP. Si LoLTV découvre une compétition internationale absente de la dernière publication Riot, ses identités sourcées sont ajoutées à la projection de lecture sans modifier rétroactivement la version Riot.

Depuis la migration `0011`, `format` accepte aussi `null` lorsque le format de série n’est pas sourcé. La vue affiche « Format inconnu » et seulement les cartes publiées ; leur nombre ne détermine ni le format ni le vainqueur de série. Les scores et états explicitement publiés restent utilisables.

La collecte LoLTV accepte aussi `cancelled` et `postponed`. Les camps et objectifs non publiés sont `null`, les compositions absentes sont vides. Les cartes peuvent fournir `updatedAt` : horodatage du flux source, ou date de lecture du document pour le HTML. La modale utilise cette date pour la carte sélectionnée. Un rafraîchissement du calendrier ne rajeunit pas les statistiques conservées ; un changement de score ou d’identifiant de carte exclut les anciens relevés live incompatibles. `GET /sources/loltv` expose les dernières exécutions, le budget partagé et le délai éventuel sans cookies de consultation.
