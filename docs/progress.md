...
- **Migrations :** création de `ml.model_versions` avec jeu, marché, segment, algorithme, hyperparamètres, version de features, dataset/hash/cutoffs, rapport/fingerprint, calibrateur, incertitude, clé/hash/taille/format de l'artefact, commit, statut, auteur, dates et motifs. Une contrainte unique partielle interdit deux champions pour le même jeu/marché/segment. Les métadonnées et suppressions sont protégées par trigger ; seuls les quatre champs de transition de statut sont réservés au workflow audité `ML-011`.
