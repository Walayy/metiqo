...
- **Commit/hash :** `c6141a6` (`fix(ui): preserve loading geometry and verify visual stability`).

## QA-003 — Accessibilité complète

- **Statut :** `DONE`.
- **Dépendances vérifiées :** `UI-011` est `DONE` ; QA-002 est figé dans `c6141a6`.
- **Fichiers créés/modifiés :** conservation du focus du filtre et synchronisation avec l'historique, régions nommées d'annonce des résultats/cotes, contraste des messages d'entraînement sombre, helper et parcours clavier desktop/mobile, exercice Owner au clavier, checklist manuelle et rapport de preuves.
- **Migrations :** aucune.
- **Commandes/tests exécutés :** tests initialement en échec pour la perte du focus, l'absence d'annonce de cote et le contraste de confirmation sombre à 3,56:1 ; correction puis parcours paper/mapping/modèles au clavier, 36 analyses WCAG A/AA sur neuf pages et quatre analyses des messages d'action, vérification manuelle dans le navigateur intégré et exercice Owner sur PostgreSQL dédié. Suite Playwright complète, TypeScript strict incluant les tests navigateur, ESLint et format/orthographe.
- **Résultat exact :** les 82 tests Playwright mock passent en 241,65 secondes, sans nouvelle tentative, avec les deux parcours Owner réservés à leur exercice séparé. Cet exercice PostgreSQL passe en 57,52 secondes avec ses deux parcours desktop/mobile et le bootstrap CLI réel. Les douze nouveaux tests couvrent les chaînes clavier et 36 analyses des pages plus quatre analyses des messages d'action, sans violation WCAG détectée. Le retour dans l'historique détecte aussi un grade conservé après effacement : la synchronisation est corrigée, puis toute la suite est relancée avec succès. Les comparaisons visuelles préexistantes passent sans régénération. TypeScript, ESLint, format et orthographe passent. Le rapport et les empreintes sont archivés dans `docs/evidence/qa-003/report.json` ; la portée exacte de la revue manuelle figure dans `docs/accessibility.md`.
- **Blocker éventuel :** aucun identifié. Les annonces sont vérifiées dans l'arbre d'accessibilité et les régions ARIA ; aucun lecteur d'écran vocal externe n'a été utilisé. Les mutations métier de cette suite restent des fixtures mock, à l'exception de la session Owner réellement persistée.
- **ADR éventuel :** pas de remontage du formulaire lors de la navigation ; noms de régions distincts et annonces polies atomiques. Les parcours rejouent l'approbation canonique de la revue mock avec la même clé d'idempotence, sans modifier les validations du serveur réel.
- **Commit/hash :** `342212f` (`fix(ui): complete keyboard paths and accessible live feedback`).

## QA-004 — Résilience réseau et erreurs

- **Statut :** `DONE`.
- **Dépendances vérifiées :** `MCK-007`, `OE-023`, `ODD-009` et `OPS-006` sont `DONE` ; QA-003 est figé dans `342212f`.
- **Fichiers créés/modifiés :** requêtes navigateur bornées et erreurs HTTP typées, conservation des lectures autorisées dans les panneaux, reprises indépendantes, indicateur réseau dans le shell, distinction entre décision absente et service indisponible, limites des connexions API PostgreSQL et réponse temporaire structurée. Suites réseau, fixtures de transport partagées, tests PostgreSQL/provider et documentation de résilience.
- **Migrations :** aucune.
- **Commandes/tests exécutés :** scénarios initialement en échec pour les lectures masquées, le 503 affiché comme une décision absente, l'absence d'état hors ligne, la panne SQL non récupérable et les délais SQL initialement illimités. Tests d'annulation de requête PostgreSQL et de remplacement de connexions réellement interrompues, ingestion d'octets validés puis timeout injecté au client du transport public, gate ingestion complet, suite navigateur complète, composants, lint/types et OpenAPI. La première suite complète détecte aussi le déplacement du contenu par l'annonce hors ligne ; celle-ci utilise désormais l'espace du badge du shell, puis les tests du graphe et le catalogue passent de nouveau.
- **Résultat exact :** 62 tests Python passent en 149,62 secondes, 27 tests composants passent et mypy valide 420 sources. La suite Playwright finale passe avec 98 tests en 242,56 secondes, sans nouvelle tentative ; ses seize scénarios de résilience passent et les références visuelles restent identiques. Les deux tests Owner sont exécutés dans leur exercice PostgreSQL séparé, qui passe en 40,51 secondes. Les captures hors ligne desktop/mobile sont inspectées ; l'indicateur reste dans le shell sans masquer les contrôles ni déplacer le graphe. Types TypeScript, ESLint, format/orthographe et OpenAPI passent. Le rapport, les empreintes et les captures sont archivés dans `docs/evidence/qa-004/`.
- **Blocker éventuel :** aucun pour les scénarios réseau identifiés. Les preuves de transport réel synthétique ne constituent aucune donnée financière ; le timeout public est injecté au client et isolé de la sélection du miroir. La recette de release reste à terminer.
- **ADR éventuel :** une panne transitoire conserve la dernière lecture avec avertissement, tandis qu'un refus de permission ou de session ferme l'accès au contenu correspondant. Aucun POST ou transaction SQL n'est rejoué automatiquement. Les délais SQL s'appliquent au processus HTTP et laissent les travaux longs au worker.
- **Commit/hash :** `89a6f4a` (`fix(resilience): retain authorized reads and bound network failures`).

## QA-005 — CI complète normative

- **Statut :** `DONE` ; CI normale et exercice négatif vérifiés sur le même commit propre.
- **Dépendances vérifiées :** `OE-024`, `FEAT-013`, `ML-017`, `VAL-009`, `PAP-010`, `OPS-010`, `QA-002` et `QA-003` sont `DONE`. QA-004 est figé dans `89a6f4a`.
- **Fichiers créés/modifiés :** orchestration réelle de l'entraînement en file, lecture d'un job et version produite, suivi navigateur avec clé conservée après réponse perdue, visibilité des versions bloquées, provenance Git des images, cutoff d'annulation avant publication, filtre SQL des signaux par événement, explications d'admissibilité alignées sur le bouton paper, refus d'un changement d'identité entre signal et entrée paper, port HTTPS interne du gateway, workflow CI complet et exercice négatif, vérificateur de preuves et documentation.
- **Migrations :** aucune nouvelle migration ; les données et contrats sont étendus sans remplacer les snapshots ni les décisions existantes.
- **Commandes/tests exécutés :** tests en échec avant raccordement de l'entraînement, avant arrêt de publication annulée, avant filtrage par événement, avant refus du changement d'équipe et avant correction du port HTTPS. Puis tests PostgreSQL, navigateur, configuration, scan officiel et reconstruction de toutes les images. L'exercice négatif copie le code, affaiblit les deux gardes de cutoff et vérifie l'échec de la vraie suite anti-fuite sans modifier le checkout.
- **Résultat local intermédiaire :** quatre tests worker/modèles passent en 26,36 secondes, huit tests paper en 37,36 secondes, cinq tests de filtres et lectures SQL en 25,34 secondes et trois configurations TLS vérifié en 39,17 secondes. Les quinze tests de configuration/provenance/gate passent en 8,28 secondes. TypeScript, mypy sur 427 sources, lint/format/orthographe et contrats générés passent. La suite Python complète exécute 616 tests sans exclusion : 615 passent, une ancienne liste de routes OpenAPI omet le nouveau détail de job ; cette attente est corrigée et les cinq tests OpenAPI passent en 7,01 secondes. La CI distante doit encore rejouer la suite entière sur le commit propre.
- **Sécurité :** le scan `data/security/20260907T153432Z-2256f27b/summary.json` passe sur les cinq images : 680 fichiers, aucune fuite ni vulnérabilité bloquante, aucune exception de vulnérabilité, 360 occurrences non bloquantes conservées. Le premier refus détectait deux checksums du rapport QA-004 ; leurs octets sont recalculés et la règle de faux positif exige le chemin et les valeurs exacts. Les tests prouvent que d'autres valeurs et chemins restent détectés. Le build local modifié porte explicitement une provenance de commit absente ; seul un checkout propre peut produire les images de release identifiées.
- **Navigateur final :** les 103 parcours sélectionnés passent sans échec, exclusion ni nouvelle tentative. Les sept tests finaux des modèles passent en 32,80 secondes, avec un contrôle supplémentaire de refus de permission puis reprise explicite. Les deux captures archivées sont inspectées : entraînement terminé sans promotion automatique et version bloquée sans bouton de promotion. Les 31 tests composants passent. Les derniers contrôles de types et lint passent après la correction de permission.
- **Clôture distante :** le commit `467a4aa3a5d187fa877663ed69781995120caea7` passe les cinq jobs du run `34142924516` : 616 tests Python et 104 parcours navigateur, sans échec ni exclusion. Les builds propres, la sécurité et le benchmark SQL passent. Le scan conserve 360 occurrences non bloquantes, sans fuite ni vulnérabilité bloquante. Le run négatif `34142754544` sur ce même commit échoue sur trois tests temporels et bloque le gate final. Les artefacts ont été téléchargés et vérifiés ; URLs, états des jobs et empreintes figurent dans `docs/evidence/qa-005/remote-ci.json`. Le rapport local antérieur conserve explicitement ses limites. QA-007 doit encore valider la recette du MVP intégrant QA-006.
- **Première CI distante :** `25649ed` est publié sur la branche dédiée. Le run négatif `34141329202` injecte la faute puis échoue sur trois tests critiques ; le gate final refuse la release. Le run normal `34141407036` détecte une différence d'import avec le lanceur `pytest` Linux. Les cibles Make utilisent désormais `python -m pytest`, comme les validations locales et le job runtime ; la nouvelle preuve verte reste nécessaire.
- **Écarts Linux corrigés :** le premier runtime exécute 616 tests sans exclusion, avec 614 succès et deux échecs de fixtures de volumes. Les fichiers sont préparés pour l'UID réel 10001, lus sous cette identité et rendus au runner après le test ; les permissions de production restent identiques. Les deux exercices passent localement en 31,77 secondes. Le navigateur exécute 104 parcours, avec 103 succès : la trace attribue l'unique exception à l'observateur CLS du test, qui traitait un nœud texte comme un élément HTML. La correction conserve chaque déplacement dans la mesure ; le parcours retardé et les comparaisons ciblées passent de nouveau.

## QA-006 — Checklist conformité et release gates

- **Statut :** `DONE` pour le gate du MVP personnel ; aucune autorisation publique ou commerciale n'est accordée.
- **Dépendances vérifiées :** `ODD-006` et `OPS-005` sont `DONE`.
- **Fichiers créés/modifiés :** audience de publication et portes typées dans la configuration API/worker, validation de preuves privées approuvées manuellement, commande de contrôle, route de conformité sans dépendance SQL, écran Paramètres, mention d'absence de gain garanti, scanner des promesses de gain, configuration Compose et checklist des sources officielles.
- **Migrations :** aucune.
- **Commandes/tests exécutés :** tests initialement en échec pour les flags absents, la preuve manuelle et l'écran vide. Puis tests de configurations personnelles et publiques, manifeste incomplet, dates, périmètre, empreinte, altération, commande réelle et contrats, tests du scanner, build Next et suite Playwright complète. La première suite révèle le déplacement du nouveau footer et un ancien test qui confond la négation d'une promesse avec une promesse : le footer suit désormais la zone de hauteur minimale et le test autorise uniquement la phrase exacte de non-garantie.
...
- **Statut :** `DONE` pour le MVP personnel ; les 22 critères passent sur le commit de release vérifié.
...
- **Travail restant :** aucun pour cette recette du MVP personnel. P10/P11 ne sont pas lancés ; les autorisations externes restent soumises aux portes NO-GO.
- **Premier exercice réel :** `make mock-demo` passe sur `891c623` avec quatre services sains, migrations appliquées, sept pages accessibles et gates NO-GO. Le projet et ses volumes neufs sont supprimés ; les sources restent identiques. Le rapport `data/acceptance/startup-84de1eacbe94/report.json` conserve les versions. Une tentative de recette avec la CI verte de QA-005 est effectivement refusée car son SHA est ancien. L'examen des labels Docker révèle que Compose change l'identifiant des images lors du build sous le projet temporaire : le démarrage précède désormais les tests runtime et le scan final vérifie leurs identifiants exacts. Onze tests de preuves/CI passent après ce contrôle supplémentaire.
- **Préparation PostgreSQL corrigée :** le run `34145331716` passe la qualité et les 106 parcours navigateur, puis s'arrête avant les tests runtime : la sonde sur socket accepte le serveur temporaire d'initialisation juste avant son arrêt. La sonde CI attend désormais TCP, ouvert par le serveur final utilisé par les tests. Le run négatif `34145818910` sur `11cc88e` bloque bien le cutoff et le gate ; la paire de runs sur le commit corrigé clôture la recette ci-dessous.
- **Clôture vérifiée :** `make acceptance CI_RUN=34146337077 NEGATIVE_CI_RUN=34146257438` retourne 0 le 7 septembre 2026. Les cinq jobs du [run normal](https://github.com/Walayy/metiqo/actions/runs/34146337077) passent sur `5eec0ea753ce940dfc6a6bed8c8397b728d91392`. Les 634 tests Python passent en 501,046 secondes ; les 106 parcours navigateur passent, sans échec ni test sauté. Le [run négatif](https://github.com/Walayy/metiqo/actions/runs/34146257438) du même commit échoue sur trois tests après l'injection de fuite temporelle et bloque le gate final.
- **Preuves runtime :** `make mock-demo` démarre effectivement un projet Compose sans ressource préexistante : quatre services sains, sept pages accessibles, readiness et migrations vérifiées, portes NO-GO et Stake désactivé. Le projet et ses volumes sont supprimés, les sources restent identiques. Les cinq identifiants d'images correspondent exactement au scan final. Les douze routes du benchmark retournent uniquement 200 sur cent lectures chacune ; le p95 maximal est de 54,761 ms. Le gate sécurité passe sans fuite ni vulnérabilité bloquante, et conserve les occurrences non bloquantes dans ses preuves.
- **Archivage :** les deux archives GitHub sont téléchargées et leurs digests vérifiés. Les rapports JUnit, de démarrage, de performance et de sécurité, les logs du refus critique et les 22 résultats sont conservés dans `docs/evidence/qa-007/5eec0ea/`. L'index relie chaque copie brute à son empreinte et au chemin de preuve original. La revue visuelle QA-006 est vérifiée contre les sources et captures archivées. La documentation décrit désormais la file d'entraînement réellement livrée.
- **Contrôle documentaire :** les preuves brutes gardent leurs octets et sont exclues du reformatage. Deux faux positifs Gitleaks correspondent aux empreintes recalculées des rapports de scan des secrets et de l'image API ; la règle exige le chemin exact de cette archive et les deux valeurs exactes. Six contrôles effectifs prouvent que les valeurs reconnues à cet emplacement passent, tandis qu'un autre chemin ou une autre valeur est détecté. `scanner-review.json` conserve ces résultats. Les 360 occurrences de vulnérabilités non bloquantes restent archivées, sans exception de vulnérabilité.
- **Blocker éventuel :** aucun pour le périmètre personnel testé. Les fixtures ne prouvent ni performance financière ni autorisation juridique. `OE-COMMERCIAL=NO-GO`, `RIOT-PRODUCT=NO-GO` et le fournisseur Stake désactivé restent effectifs.
- **Commit/hash :** release testée `5eec0ea753ce940dfc6a6bed8c8397b728d91392` ; les commits suivants consacrés à ces preuves et à la documentation ne sont pas présentés comme ayant reçu les tests du commit de release.
