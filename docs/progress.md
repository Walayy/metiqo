...
- **Préparation PostgreSQL corrigée :** le run `34145331716` passe la qualité et les 106 parcours navigateur, puis s'arrête avant les tests runtime : la sonde sur socket accepte le serveur temporaire d'initialisation juste avant son arrêt. La sonde CI attend désormais TCP, ouvert par le serveur final utilisé par les tests. Le run négatif `34145818910` sur `11cc88e` bloque bien le cutoff et le gate ; une nouvelle paire de runs sur le commit corrigé reste nécessaire.
