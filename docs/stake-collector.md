...
Un **403/429 auxiliaire** est journalisé, avec URL expurgée, et le parcours continue tant que les données requises sont disponibles. Aucune requête directe ne rejoue ces ressources. Un refus du document principal, un challenge réellement bloquant ou une limitation explicite déclenche une pause source persistante, avec respect de `Retry-After`. Le planificateur replace la demande en attente avec sa date de reprise. Les données précédentes restent disponibles ; LoLTV et Oracle continuent indépendamment.
