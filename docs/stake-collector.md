...
Depuis la correction du 25 septembre 2026, le statut du passage Stake dépend des snapshots réellement persistés. Un passage avec au moins un snapshot est terminé, même si sa couverture est incomplète ; les événements écartés et l'étape d'un éventuel arrêt global restent visibles séparément. Une erreur sans snapshot donne un échec ; une liste vide sans erreur termine normalement. La migration `0019` requalifie les anciens passages `failed` seulement lorsqu'un relevé existe en base, conserve leur diagnostic dans `details.statusCorrection` et marque leur couverture incomplète. Elle ne reconstitue pas les raisons que l'ancien worker ne stockait pas. Les délais source continuent de bloquer tout accès prématuré. Pour les duels de joueurs sans identifiant de marché dans le DOM, le titre et la paire de joueurs sourcée forment une identité stable, indépendante de l'ordre et des cotes. Si cette preuve manque ou se répète, la rencontre n'est toujours pas publiée.
