...
- `npm run check` réussi : TypeScript, ESLint, 80 tests frontend, build, Ruff, mypy et 183 tests backend hors intégration. Le contrat frontend refuse deux relevés de même marché et phase. Le test PostgreSQL ciblé `test_match_odds.py` est présent mais a été ignoré faute de `TEST_DATABASE_URL` isolée ; l’API reconstruite a été vérifiée sur la base locale existante, sans altérer ses données.
