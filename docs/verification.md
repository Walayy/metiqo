...
- Le smoke Playwright Chromium passe localement : chargement du build mock, navigation vers Matchs et absence d’erreur JavaScript. Le workflow `.github/workflows/ci.yml` publie les quatre contextes exigés par la protection de `master` ; leur exécution distante reste à confirmer sur le commit poussé.
