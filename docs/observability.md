...
Le scheduler réel met en file `ops.alerts` toutes les cinq minutes (`ALERT_INTERVAL_SECONDS`). Le handler lit les mêmes preuves que la supervision. Il alerte si la source ou le champion n'est pas à jour, si le backlog de mapping atteint `ALERT_MAPPING_BACKLOG_LIMIT` (dix par défaut), si une anomalie bloquante reste active, ou si les sauvegardes sont absentes, périmées ou en échec. La disponibilité en lecture reste indépendante d'une alerte source.

`ops.alert_states` conserve chaque condition, sa dernière observation et sa dernière notification. Deux workers partagent un verrou transactionnel : apparition, état et événement d'audit sont atomiques. Un état actif inchangé, même avec un compteur différent, reste silencieux jusqu'au rappel, initialement six heures (`ALERT_COOLDOWN_SECONDS`). Un retour à la normale émet une seule résolution. Une nouvelle apparition après résolution est un nouvel incident et est signalée immédiatement. Une horloge antérieure à la dernière observation ne réécrit pas cet état.

Les anomalies de qualité historiques restent dans les métriques. Pour les alertes, une confirmation de contenu ou un chargement réussi postérieur, sur la même source, clôt les incidents antérieurs ; un nouvel échec distant ne les efface pas. Aucun simple rafraîchissement de catalogue ne clôt une anomalie de contenu.

Les événements `alert.opened`, `alert.reminder` et `alert.resolved` sont conservés dans le journal d'audit admin avec code, acteur et trace du job. Les logs locaux annoncent les apparitions et rappels en warning, les résolutions en info. L'événement PostgreSQL reste consultable si le processus s'arrête avant l'émission du log. Aucun message externe, abonnement ou compte SaaS n'est nécessaire. Si le worker ne tourne pas ou si sa file est occupée, les contrôles attendent son exécution ; ces alertes locales ne sont pas un service de supervision indépendant de la stack.
