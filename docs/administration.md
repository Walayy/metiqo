...
Scripts distingue les trois services : **Collectes sportives** (catalogue, Oracle et LoLTV), **Cotes Stake** et **Résultats des sélections**. La barre « Services » résume leur disponibilité et le nombre d’exécutions en cours. Son dépliage présente les trois accès aux journaux ; le dernier contact est visible dans l’en-tête du lecteur. Choisir un service ouvre son journal ; « Voir le journal » dans une exécution l’ouvre directement sur son identifiant. Le retour à l’historique conserve l’exécution dépliée. Les événements de service (démarrage, arrêt et incidents de planification) restent visibles même sans exécution associée.

Le lecteur utilise le dialogue commun : feuille avec poignée sur téléphone, fenêtre centrée sur tablette et ordinateur. Le sélecteur d’exécution, les filtres de gravité et la recherche restent sur cette même surface. Chaque ligne affiche l’heure, la gravité et le message ; un dépliage animé révèle la date complète, le script, l’étape, la rencontre et le contexte technique. Les jours sont regroupés en heure de Paris. Le compteur d’incidents reste visible quels que soient les filtres ; son détail se déplie à la demande. Une ligne consultée suspend le suivi automatique, comme un défilement vers le haut.
