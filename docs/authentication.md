...
L’inscription crée toujours un compte `user` par défaut SQL. Depuis la migration `0004`, l’API peut modifier uniquement `role` et `disabled` dans l’espace Admin, après contrôle de session, du rôle courant et de l’origine. Un changement de rôle/statut révoque toutes les sessions. Un compte suspendu ne peut plus valider de connexion, même avec un code correct. L’API n’a pas le droit de supprimer un compte. Le worker n’accède ni aux quatre tables d’authentification ni à l’audit Admin. Les données esport restent en lecture pour l’API. Voir [administration.md](administration.md).

Migration `0005` (demande du 16 septembre) : `metiquo@admin.fr` reçoit `admin`, `metiquo@user.fr` reçoit `user`. Le provisionnement crée uniquement les comptes absents sans `verified_at`. Les comptes existants conservent identité, date de vérification et suspension ; leurs sessions sont révoquées si le rôle change. La connexion requiert toujours la preuve email. Les inscriptions ordinaires restent créées après vérification, avec le rôle SQL par défaut `user`. Le downgrade ne supprime pas de compte et ne rétablit pas des droits qui auraient changé depuis.
