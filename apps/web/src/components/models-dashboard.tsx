...
function MetricComparison({ model, metric }: Readonly<{ model: ModelSummary; metric: string }>) {
  ...
  const baseline = Number(model.baselineMetrics[metric] ?? 0);
  const summary = `${metricLabel(metric)} : modèle ${formatDecimal(value)}, baseline ${formatDecimal(baseline)}. Une valeur plus basse est préférable.`;
  const maximum = Math.max(value, baseline, 0.01);
        ...
        {(
          ...
          return (
            <div className="grid grid-cols-[5rem_1fr_auto] items-center gap-2 text-xs" key={label}>
              ...
              <span className="font-semibold tabular-nums">{formatDecimal(numericValue)}</span>
            </div>
          );
        })}
      </div>
          ...
          <div className="grid min-w-0 gap-6">
            ...
            <Panel icon={<ShieldCheck className="size-4.5" />} title="Champions actifs">
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {champions.map((model) => (
                  <ModelCard key={model.modelVersionId} model={model} />
                ))}
              </div>
            </Panel>
            ...
            <Panel icon={<GitCompareArrows className="size-4.5" />} title="Challengers">
              {challengers.length === 0 ? (
                <p className="rounded-lg border border-border-subtle bg-surface-muted p-4 text-sm text-ink-secondary">
                  Aucun challenger dans le catalogue mock courant. Aucune comparaison artificielle
                  n’est créée.
                </p>
              ) : (
            ...
            >
              ...
              <p className="flex gap-2 rounded-lg border border-amber-300 bg-amber-50 p-3 text-xs leading-5 text-amber-900 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-100">
                ...
                Tous les backtests mock contiennent moins de 500 observations. Les résultats sont
                signalés comme faible échantillon et ne suffisent pas à une promotion réelle.
              </p>
            </Panel>
