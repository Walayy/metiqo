...
Cible : victoire de l'équipe A dans la **première partie**, avant la série, contre B, dans la même compétition régionale LEC ou LCK d'équipes premières. Entraîner et évaluer sur cette même cible. Ne pas utiliser les résultats des parties 2 à 5 pour enrichir silencieusement la cible de la V1.
...
Exiger les deux équipes, une ligue prise en charge, la partie 1 explicitement identifiée, des données validées et des dates comparables. Écarter doublons, matchs partiels, forfaits/remakes non interprétables et données hors cible selon le contrat d'import. Une partie canonique correspond à une observation statistique, pas à deux équipes comptées comme deux matchs.
...
Présenter le nombre de parties, la période, la couverture avant/après abstention et les résultats par ligue. Pour la calibration, répartir les prédictions en cinq groupes de taille proche, afficher probabilité moyenne, fréquence de victoire et effectif ; calculer l'écart absolu moyen pondéré. Ne pas confondre cet écart avec l'erreur d'une rencontre particulière.

Conditions initiales pour le statut **qualifié** : Brier et log-loss meilleurs que la référence 50/50, au global et dans chaque ligue activée ; écart de calibration pondéré ≤0,08 au global ; volumes minimaux précédents respectés. Ajouter une estimation de l'incertitude de la différence de log-loss : 2 000 rééchantillonnages par semaines UTC entières, graine enregistrée, au moins 12 semaines de test, intervalle percentile à 95 %. Exiger une borne supérieure négative au global. L'intervalle est approximatif et concerne la performance moyenne, pas « les chances à 95 % » du match.
...
Probabilité calculable : chaque équipe dispose d'au moins **15 premières parties validées dans les 180 derniers jours**, dont une au cours des 45 derniers jours ; ligue et identité résolues ; dernier contrôle complet réussi d'Oracle’s Elixir datant de **36 h au plus**. Ces bornes initiales sont des choix de prudence. Une nouvelle équipe peut être incluse dans le classement d'entraînement sans devenir immédiatement analysable.
...
**Opportunité potentielle** seulement si toutes les conditions sont satisfaites : modèle qualifié et encore valide, historique suffisant, source et marché frais selon le skill Stake, identification exacte, p compris entre 0,10 et 0,90, EV ≥5 %, et `(p−0,03)×d−1 > 0`. Cette dernière baisse de trois points est un **test de sensibilité arbitraire**, pas une borne de confiance. Afficher ce libellé exact dans le détail. Ne pas transformer ces marges en promesse de sécurité.
...
| Marché de série, académie homonyme, partie 2 ou état live | Analyse cible refusée, même si les noms semblent correspondre. |
