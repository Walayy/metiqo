...
La V1 ne charge que les fichiers nécessaires aux **24 derniers mois**, pour toutes les compétitions LoL publiées par Oracle’s Elixir, mais découvre toutes les années du catalogue pour reconnaître automatiquement les nouveaux fichiers. Aucun filtre LEC/LCK, régional ou de niveau d'équipe n'est appliqué. Un changement d'année civile déclenche la découverte, pas la fabrication d'une URL. Les fichiers plus anciens conservés comme archives ne sont pas présentés comme surveillés en continu. Relever la couverture et la dernière partie disponible par compétition ; la couverture de toutes les compétitions souhaitées reste à mesurer, jamais à présumer.
...
Isoler les enregistrements d'équipes : **ne pas confondre les lignes de joueurs avec des parties supplémentaires**. Construire une seule partie canonique à partir de deux équipes différentes et de résultats complémentaires 0/1. Vérifier identifiant unique, date plausible, compétition identifiée, numéro de partie positif. Découvrir les compétitions à partir des valeurs source, sans liste fermée de ligues ; conserver leur libellé et leur saison/édition lorsqu'elles sont disponibles. Une identité ambiguë exige un diagnostic, pas une exclusion géographique implicite. Les équipes premières et leurs académies restent distinctes. Les remakes, forfaits et observations partielles doivent être reconnus par les champs effectivement documentés ; si leur statut n'est pas déterminable, les exclure de l'apprentissage et en compter le motif.

Une colonne facultative supplémentaire peut être conservée sans casser l'import. Une colonne requise absente, renommée sans mapping approuvé ou changeant de type met la révision en quarantaine. Toutes les compétitions LoL présentes sont admissibles au périmètre ; seules les premières parties satisfaisant les autres critères alimentent le modèle. Les autres parties ne sont pas considérées comme corrompues du seul fait de leur numéro. Une partie incohérente appartenant au périmètre cible bloque la promotion du fichier concerné plutôt que de réduire silencieusement le dataset.
...
| Fichier courant | Identifiant réellement publié, nom, date de contrôle, taille reçue, SHA-256, en-têtes, lignes brutes et parties canoniques, maximum des dates après lecture complète ; inventaire et volumes par compétition, sans filtre LEC/LCK. **Toutes ces valeurs sont actuellement non mesurées.** |
