...
class StakeScrapeRun(Base):
    """État durable d'une collecte, y compris des refus d'accès et de la temporisation."""

    __tablename__ = "stake_scrape_runs"
    __table_args__ = (
        CheckConstraint("status IN ('operational', 'partial', 'blocked', 'failed')", name="status"),
        CheckConstraint("event_count >= 0", name="event_count"),
        Index("ix_stake_runs_finished", "finished_at"),
        {"schema": ODDS_SCHEMA},
    )
...
class StakePageCapture(Base):
    """Marchés visibles d'un événement, conservés ensemble sans normalisation spéculative."""

    __tablename__ = "stake_page_captures"
    __table_args__ = (
        UniqueConstraint("sha256", name="uq_stake_capture_sha256"),
        Index("ix_stake_capture_event_time", "provider_event_id", "observed_at"),
        {"schema": ODDS_SCHEMA},
    )
    id: Mapped[UUID] = mapped_column(primary_key=True)
    run_id: Mapped[UUID] = mapped_column(ForeignKey("odds.stake_scrape_runs.id"), nullable=False)
    provider_event_id: Mapped[str] = mapped_column(String(255), nullable=False)
    starts_at: Mapped[datetime] = mapped_column(UtcDateTime(), nullable=False)
