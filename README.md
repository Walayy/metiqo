...
Le worker utilise le navigateur Patchright pour lire le HTML rendu des pages LoL SofaScore de la fenêtre J−7 à J+7. Le réseau Docker `ingestion` est dual-stack : Chromium peut utiliser la sortie IPv6 de l’hôte au lieu d’être limité au chemin IPv4, qui peut être refusé par le frontal de la source. Pour les portraits, un enrichissement limité des fiches de cartes utilise `curl-cffi` avec une empreinte TLS Chrome afin de récupérer le nom officiel associé à l’icône ; cet enrichissement est facultatif et ne remplace jamais le relevé rendu. Le worker ne visite pas Stake. Le script `sofascore-matches` est planifié toutes les cinq minutes par défaut ; l’administrateur peut modifier ou suspendre cette fréquence dans **Gestion → Scripts & planifications**. Une exécution manuelle est possible avec `npm run data:sofascore:sync` ou `metiquo-worker sync-sofascore-matches`.

Le scraper applique une discipline de trafic : délai aléatoire borné entre navigations, cache de la découverte des journées pendant cinq minutes, rafraîchissement prioritaire des rencontres live, absence de revisite des matchs terminés dont les cartes sont déjà acquises, et circuit breaker de quinze minutes après un `403` ou `429`. Un résultat terminé encore dépourvu de cartes reste donc réparable lors d’une collecte suivante. Ces garde-fous réduisent la charge sans modifier le cron métier ; une source qui demande l’arrêt n’est pas martelée et aucune donnée de repli n’est inventée.

Les rencontres sont réconciliées avec les équipes, ligues et rencontres existantes par identifiants de provenance, noms normalisés, sens de l’affiche, ligue et proximité horaire. Un identifiant SofaScore stable reste publiable même si aucun autre fournisseur ne peut encore être rapproché : l’ambiguïté empêche uniquement la fusion entre fournisseurs, pas l’affichage de la rencontre sourcée. Après un redémarrage, les identifiants terminés ou planifiés à plus de trente minutes déjà présents en base ne sont pas relus inutilement ; le worker atteint ainsi les nouvelles journées avant de rafraîchir les directs et les coups d’envoi proches. Chaque relevé est conservé avec son URL, son empreinte et son horodatage dans `match_source_links` et `match_snapshots`; cela permet de relier Oracle’s Elixir, SofaScore et de futurs fournisseurs sans faire confiance au nom seul. L’API expose le dernier relevé dans `/api/v1/matches` et l’état des collectes dans `/api/v1/sources/sofascore`.

Les scores et statuts live viennent du dernier relevé SofaScore. L’heure « Relevé à » correspond à la dernière observation du lien source, même si le payload est identique et qu’aucun nouveau snapshot n’a donc été créé. Sur chaque onglet de carte rendu, le worker publie progressivement les objectifs et les cinq joueurs de chaque camp avec leur niveau, K/D/A, CS et or ; une carte terminée peut ainsi apparaître pendant que la série continue. Les images officielles de champions sont conservées et leur nom est résolu depuis la fiche de lineup lorsque SofaScore l’expose ; si l’enrichissement est indisponible, l’image reste affichée et le nom demeure explicitement inconnu. Une durée ou des bans non identifiables restent `null`/vides et l’interface les annonce comme indisponibles au lieu de masquer le reste de la carte. Pour un match terminé, Oracle’s Elixir reste une source historique complémentaire lorsque ses cartes complètes sont présentes. L’API fusionne le statut live le plus récent avec le meilleur relevé de cartes non vide : un relevé ultérieur sans détail ne peut pas effacer l’historique déjà acquis. Aucune statistique absente n’est inventée.
