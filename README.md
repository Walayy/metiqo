...
La correction de reprise SofaScore du 21 septembre ajoute les migrations **0009–0010** (`alembic upgrade head`) : délai 403/429 et état de découverte persistants, budget partagé des navigations et téléchargements de logos, navigateur au repos entre les passages, files Oracle/SofaScore indépendantes et index des recherches historiques Oracle. Les cadences configurées et la fenêtre J−7/J+7 sont conservées. Les logos manquants utilisent une icône accessible sans initiales. Les détails historiques complets d’Oracle peuvent être complétés par les champs SofaScore manquants. Voir le [diagnostic, les vérifications et les limites du rapprochement](docs/sofascore-resilience-audit.md), notamment le fuseau Oracle encore non confirmé.

Audit de la branche `v1-ajout-live` du 21 septembre 2026 : [corrections, preuves et limites](docs/review-v1-ajout-live.md). Les commandes de vérification sont `npm run check`, `npm run build:api` et `uv run --frozen pytest` avec `TEST_DATABASE_URL` vers une base isolée dont le nom finit par `_test`. Le dernier appel inclut les tests PostgreSQL que `npm run check` exclut.

La migration `0008` conserve les retours à un état déjà observé après une correction de la source ; appliquer `alembic upgrade head` avant de démarrer le worker corrigé. `METIQUO_ORACLE_DATE_TIMEZONE` est facultatif : laisser vide tant que le fuseau des dates Oracle sans offset n’a pas été vérifié. Les CSV sont toujours importés, mais ces dates ne permettent alors aucun rapprochement temporel automatique. Aucune valeur UTC n’est supposée.
...
Le détail d’une carte utilise des portraits ronds pour les bans, alignés avec chaque équipe sur ordinateur et regroupés par équipe sur mobile. Les noms se consultent au survol ou dans une bulle au clic, au clavier et au toucher. Un tableau commun compare éliminations, or et objectifs ; les joueurs conservent champion, rôle, niveau, K/D/A, CS et or dans deux colonnes sur ordinateur, puis une seule sur petit écran. Les inconnues restent `—`, avec une légende, et seules les équipes dont le camp est publié portent un libellé bleu/rouge. Aucun changement de source ni lancement de collecte n’est nécessaire à cet affichage.
...
Le worker utilise le navigateur Patchright pour lire le HTML rendu des pages LoL SofaScore de la fenêtre J−7 à J+7. Le réseau Docker `ingestion` est dual-stack : Chromium peut utiliser la sortie IPv6 de l’hôte au lieu d’être limité au chemin IPv4, qui peut être refusé par le frontal de la source. La collecte lit uniquement le DOM et le JSON SSR présent dans le HTML : aucun appel API direct SofaScore, aucune lecture des corps JSON interceptés. Le site peut effectuer ses propres requêtes pour se rendre. Les onglets sont lus après leur chargement. Les bans et les champions sont extraits du DOM ; un portrait sans libellé est comparé au catalogue Riot local, sans requête réseau supplémentaire. Les cas ambigus gardent leur image et un nom inconnu ; les archives restent conservées. Voir le [diagnostic des 403 et la suppression des appels API](docs/sofascore-403-diagnosis.md). Le worker ne visite pas Stake. Le script `sofascore-matches` est planifié toutes les minutes par défaut ; l’administrateur peut modifier ou suspendre cette fréquence dans **Gestion → Scripts & planifications**. Une exécution manuelle est possible avec `npm run data:sofascore:sync` ou `metiquo-worker sync-sofascore-matches`.

Aucun plafond de journées ou de matchs ne tronque une exécution : toutes les pages dues de J−7 à J+7 sont parcourues, une seule fois chacune. Les anciens paramètres `SOFASCORE_MAX_EVENTS`, `SOFASCORE_EVENTS_PER_RUN` et `SOFASCORE_LISTING_DAYS_PER_RUN` sont supprimés. Le worker réutilise un seul Chromium et un profil persistant dans le volume dédié `sofascore_browser` : cookies et cache HTTP survivent aux redémarrages. Les requêtes ne sont plus routées/interrompues individuellement, ce qui préserve le cache HTTP et le chargement normal du site. Une pause de **15 à 30 secondes après chaque lecture** précède la navigation suivante ; entre onglets, la pause de 4 à 8 secondes s'applique aussi après un clic échoué. La page est mise au repos pendant les pauses et entre cycles pour arrêter ses actualisations de fond.

La découverte utilise des délais de cache ajustés : aujourd'hui 3 minutes, futur 15 minutes, passé 1 heure. Les matchs live ont un intervalle minimal de 2 minutes ; un match programmé proche devient à relire après 3 minutes, un match plus distant après 15 minutes. Un résultat terminé avec cartes devient à relire après 1 heure, sans cartes après 5 minutes. Ces échéances sont traitées au prochain passage disponible : elles ne garantissent pas une collecte à la seconde près et ne modifient pas le cron enregistré. Les pages et identifiants déjà tentés dans un cycle sont dédupliqués, même via des liens différents. Les cartes terminées d'une série encore live sont réutilisées par la publication, sans être présentées comme une nouvelle observation. Le premier `403` ou `429` observé coupe le réseau et ferme les pages ; le cycle s'arrête et conserve les données précédentes. Le délai progressif persistant et `Retry-After` restent respectés. Un passage peut durer plus longtemps que le cron, sans exécution concurrente ni plafond de couverture.

Ces modifications répondent au [diagnostic du 403 de 13:22:54](docs/sofascore-403-2026-09-21-1322.md). Les paramètres `METIQUO_SOFASCORE_*` sont détaillés dans [le guide des collecteurs](docs/collectors.md#session-persistante-et-cache-renforcé--21-septembre-2026). Le profil local hors Docker utilise `METIQUO_SOFASCORE_BROWSER_PROFILE_DIR` (défaut `.cache/backend/sofascore-browser`) et ne doit pas être partagé par deux Chromium simultanés. Après le « go » et la relance à 15:43, un [nouveau 403 à 15:46:40](docs/sofascore-403-2026-09-21-1546.md) survient pendant la découverte des journées. **Le worker a été arrêté pour ce diagnostic ; le scraping DOM ne garantit pas l'absence de blocage.** La perte du journal et du délai de ce refus a été reproduite et corrigée hors ligne ; cette dernière correction n'est pas encore déployée.

Les rencontres sont réconciliées avec les équipes, ligues et rencontres existantes par identifiants de provenance, noms normalisés, sens de l’affiche, ligue et proximité horaire. Un identifiant SofaScore stable reste publiable même si aucun autre fournisseur ne peut encore être rapproché : l’ambiguïté empêche uniquement la fusion entre fournisseurs, pas l’affichage de la rencontre sourcée. Après un redémarrage, les identifiants terminés et complets ne sont pas relus inutilement ; les échéances de fraîcheur restent applicables aux matchs planifiés et les directs connus sont prioritaires parmi les fiches dues. Chaque relevé est conservé avec son URL, son empreinte et son horodatage dans `match_source_links` et `match_snapshots`; cela permet de relier Oracle’s Elixir, SofaScore et de futurs fournisseurs sans faire confiance au nom seul. L’API expose le dernier relevé dans `/api/v1/matches` et l’état des collectes dans `/api/v1/sources/sofascore`.

Les scores et statuts live viennent du dernier relevé SofaScore. L’heure « Relevé à » correspond à la dernière observation du lien source, même si le payload est identique et qu’aucun nouveau snapshot n’a donc été créé. Sur chaque onglet de carte rendu, le worker publie progressivement les objectifs et les cinq joueurs de chaque camp avec leur niveau, K/D/A, CS et or ; une carte terminée peut ainsi apparaître pendant que la série continue. Les images de champions et bans restent conservées même si aucun nom fiable ne peut être identifié. L’interface affiche alors « Nom indisponible ». Une durée absente reste `null` et une section de bans absente reste vide. Le catalogue visuel embarqué dans le worker se reconstruit hors ligne avec `npm run data:champions:reference` ; `npm run data:champions:sync` actualise d’abord les assets Riot puis ce catalogue. Reconstruire ensuite l’image worker pour utiliser le nouveau référentiel. Pour un match terminé, Oracle’s Elixir reste une source historique complémentaire lorsque ses cartes complètes sont présentes. L’API conserve chaque carte publiée compatible avec le dernier statut et le score sourcé : un relevé ultérieur sans détail ne peut pas effacer l’historique déjà acquis. Aucune statistique absente n’est inventée.
...
L’[audit du 15 septembre 2026](docs/oracles-elixir-audit.md) a validé la récupération des **13 CSV Oracle’s Elixir (2014–2026)** par téléchargement groupé en ZIP, malgré le quota bloquant les liens individuels. L’archive complète contenait 848,6 Mo de CSV et 1 223 472 lignes vérifiées. Les scripts temporaires de cet audit ont été supprimés. À la demande suivante, cette méthode a été implémentée dans le worker permanent, avec validation, empreintes SHA-256 et import atomique. Le mode API peut projeter les cartes Oracle uniquement après rapprochement non ambigu et vérification du fuseau et du format. Cette voie dépend toujours du service Google ; elle n’offre pas de garantie absolue de disponibilité.
